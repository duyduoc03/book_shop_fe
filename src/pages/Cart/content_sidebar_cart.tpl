				...
				<li nh-cart-item="{$product_item_id}" nh-cart-item-quantity="{if !empty($item.quantity)}{$item.quantity}{/if}" class="cart-item clearfix">
					<div class="inner-image">
				        ...
				        <a href="{$this->Utilities->checkInternalUrl($item.url)}" title="{if !empty($item.name_extend)}{$item.name_extend}{/if}">
				            <img class="img-fluid rounded-5" src="{$url_img}" alt="{if !empty($item.name_extend)}{$item.name_extend}{/if}">
				        </a>
				    </div>

				    <div class="inner-content">
				    	{if !empty($item.name_extend)}
			                <a class="product-title block-line-clamp-1" href="{$this->Utilities->checkInternalUrl($item.url)}">
			                    {$item.name_extend|escape}
			                </a>
		                {/if}
	...
	{if !empty($items)}
		<div class="entire-bottom-minicart">
			...
			<div class="mini-cart-btn">
				<a href="/order/cart-info" class="btn btn-cart-info rounded btn-submit">
					{__d('template', 'xem_gio_hang')}
				</a>
				
				<a href="/order/info" class="btn btn-checkout rounded btn-submit">
